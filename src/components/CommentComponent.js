...
import { Icon, Button, Text, Input, Container, Item } from "native-base";
...
class CommentComponent extends BaseScreen {
    ...
    Api.get("load/comments", {
      ...
      .then((result) => {
        ...
        this.updateState({
          itemLists: lists,
          fetchFinished: true,
          itemListNotEnd: result.length > 0 ? false : true,
        });
      })
      .catch(() => {
    ...
    return (
      <View style={{ flex: 1, flexDirection: "row", padding: 10 }}>
        ...
        <View style={{ flex: 1, flexDirection: "column", marginLeft: 7 }}>
          <Text style={{ color: this.theme.blackColor }}>
            <Text style={{ color: this.theme.blackColor, fontWeight: "bold" }}>
              {item.user.full_name}
            </Text>{" "}
            {item.message}
          </Text>
            ...
            {!reply ? (
              <View style={{ flexDirection: "row" }}>
                {item.replies > 0 ? (
                  <TouchableOpacity
                    onPress={() => {
                      this.openReplies(item.id);
                    }}
                    style={{ marginRight: 10 }}
                  >
                    <Text style={{ color: this.theme.greyColor }}>
                      <Text
                        style={{
  ...
  loadReplies(paginate) {
    ...
    Api.get("load/comments", {
      ...
      .then((result) => {
        ...
        this.updateState({
          replyItemLists: lists,
          replyFetchFinished: true,
          replyItemListNotEnd: result.length > 0 ? false : true,
        });
      })
      .catch(() => {
        this.updateState({
          replyFetchFinished: true,
          replyItemListNotEnd: true,
        });
      });
  }
  ...
  submitReply() {
    if (this.state.replyText === "") {
      Toast.show(
        lang.getString("please-enter-comment"),
        Toast.LONG,
        this.theme.toast.brand
      );
      return;
    }
        ...
        {this.state.viewType === 1 ? (
          <View style={{ flexDirection: "column", flex: 1 }}>
                ...
                ListEmptyComponent={
                  !this.state.fetchFinished ? (
                    ...
                    <EmptyComponent
                      paddingTop={70}
                      icon="cup"
                      text={lang.getString("no_comments_found")}
                    />
                  )
                }
                ListFooterComponent={
            ...
            <View
              style={{
                height: 60,
                borderTopColor: this.theme.greyHeaderBg,
                borderTopWidth: 1,
                padding: 10,
                flexDirection: "row",
              }}
              ...
              {this.isLoggedIn() ? (
                <View style={{ flex: 1, flexDirection: "row" }}>
                  ...
                  <Item rounded style={{ flex: 1, marginLeft: 5 }}>
                    <Input
                      ...
                      placeholder={lang.getString("enter-comment")}
                ...
                <View style={{ alignContent: "center", flex: 1 }}>
                  ...
                  >
                    ...
                    >
                      {lang.getString("login-or-register-comment")}
                    </Text>
                  </TouchableOpacity>
                </View>
              )}
            ...
            <View style={{ flex: 1 }}>
              <FlatList
                ...
                onEndReached={() => {
                  //console.log(d.distanceFromEnd )
                  if (
                    this.state.replyItemLists.length > 0 &&
                    !this.state.replyItemListNotEnd &&
                ...
                ListEmptyComponent={
                  !this.state.replyFetchFinished ? (
                    ...
                    <EmptyComponent
                      paddingTop={70}
                      icon="cup"
                      text={lang.getString("no_replies_found")}
                    />
                  )
                }
                ListFooterComponent={
                ...
                renderItem={({ item, index }) =>
                  this.displayComment(item, index, true)
                }
              />
            </View>
            <View
                ...
                <View style={{ flex: 1, flexDirection: "row" }}>
                  ...
                  <Item
                    ...
                    style={{
                      flex: 1,
                      marginLeft: 5,
                      borderColor: this.theme.borderLineColor,
                  ...
                  >
                    <Input
                      ...
                      placeholder={lang.getString("reply")}
                      onChangeText={(t) => this.updateState({ replyText: t })}
                    />
                  </Item>
                  <Button
                    onPress={() => {
