...
class VideoComponent extends BaseScreen {
    ...
    constructor(props) {
        ...
        this.cacheFilter = (this.props.cacheFilter !== undefined) ? this.props.cacheFilter : 'video';
    ...
    loadLists(paginate) {
        ...
        if (!paginate && !this.cacheLoaded) {
            ...
            if (!this.noCache) {
                Realm.open({ path: 'videos.realm', schema: [offlineSchema] }).then(realm => {
    ...
    render() {
        return (<FlatGrid
            ...
            ListEmptyComponent={!this.state.fetchFinished ? (
                <Text />
            ) : (<EmptyComponent text={lang.getString('no_videos_found')} />)}
            renderItem={({ item, index }) => this.displayItem(item, true)} />);
    }
