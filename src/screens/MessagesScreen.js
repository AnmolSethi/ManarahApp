...
class MessagesScreen extends BaseScreen {
  ...
  loadLists(paginate) {
    ...
    if (!paginate && !this.cacheLoaded) {
      ...
      if (!this.noCache) {
        Realm.open({ path: "messages.realm", schema: [offlineSchema] }).then(
          (realm) => {
      ...
      >
        ...
        >
          ...
          <View style={{ flex: 1, marginLeft: 10, flexDirection: "column" }}>
            ...
            <Text note style={{ color: this.theme.blackColor }}>
            ...
            >
              {Time.ago(item.message.chattime)}
            </Text>
            {item.unread > 0 ? (
              <View
                style={{
