...
import { BASE_CURRENCY, MONTHLYPRODUCT_ID, YEARLYPRODUCT_ID } from "../config";
import Api from "../api";
      ...
      >
        ...
        {this.state.details === null ? (
          ...
          <Content>
            <Segment>
              <Button
                onPress={() => this.updateState({ page: "yearly" })}
                first
                active={this.state.page === "yearly"}
              >
                <Text style={{ marginLeft: 20, marginRight: 20 }}>
                  {lang.getString("yearly")}
                </Text>
              </Button>
              <Button
                onPress={() => this.updateState({ page: "monthly" })}
                last
                active={this.state.page === "monthly"}
              >
                <Text style={{ marginLeft: 20, marginRight: 20 }}>
                  {lang.getString("monthly")}
                </Text>
              </Button>
            </Segment>
            ...
            >
              {this.state.page === "yearly" ? (
                ...
                >
                  ...
                  >
                    {lang.getString("listen-to-allmusic")}
                  </Text>
                  ...
                  >
                    {lang.getString("download-allmusic")}
                  </Text>
                  ...
                  >
                    {lang.getString("listen-to-music-offline")}
                  </Text>
                  ...
                  <Button
                    onPress={() => {
                      if (
                        Platform.OS === "ios" &&
                        this.props.setup.in_app_purchase
                      ) {
                        InAppUtils.canMakePayments((canMakePayments) => {
                          if (!canMakePayments) {
                            Toast.show({
                              text: lang.getString(
                                "cant_make_payment_on_this_device"
                              ),
                              type: "danger",
                            });
                          } else {
