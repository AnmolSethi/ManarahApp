...
export default class Player {
    ...
    TrackPlayer.setupPlayer({
      ...
      .then(() => {
        ...
        if (!this.eventsListened) {
          ...
          TrackPlayer.addEventListener("remote-stop", async (data) => {
            this.event("stop");
          });
        ...
        Realm.open({ schema: [trackSchema] }).then((realm) => {
          let tracks = realm
            ...
            .filtered('id="' + parseInt(this.track.id) + '"');
  ...
  paginate() {
    ...
    this.currentPlayListOffset =
      this.currentPlayListLimit + this.currentPlayListOffset;
    Api.get("load/tracks", {
